...
//            }
            ...
            if ((host && P1_score == 4)  || (host && P1_score == 1 && pres_mode)){
                msg_send = "winner_host?:true"
            }
            if ((!host && P2_score == 4) || (!host && P2_score == 1 && pres_mode)){
...
//        }
    ...
    func game_arena_create(coordinates_to_lazy_to_remove: SCNVector3, node_passed: SCNNode)  {
        ...
        if (change_arena == true){
            ...
            //MARK: Goal Shit
            let cylender = SCNCylinder(radius: CGFloat((0.2 * mult)), height: 0.02 )
...
//            planeNode.physicsBody?.contactTestBitMask    =  bodytype.ball.rawValue
    ...
    func end_game()  {
        ...
        DispatchQueue.main.async {
            ...
            else if(host_win == "true" && self.host){
                display = "YOU WIN!!!"
...
//        }
    ...
    @IBAction func pres_mode_btn(_ sender: Any) {
        pres_mode = true
        mult = 2
    }
